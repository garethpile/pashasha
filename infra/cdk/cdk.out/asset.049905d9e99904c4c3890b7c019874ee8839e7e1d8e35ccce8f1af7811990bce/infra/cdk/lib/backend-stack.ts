...
export class SecurityGuardPaymentsBackendStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: SecurityGuardPaymentsBackendStackProps = {}) {
    ...
    const createCognito = new sfn.Pass(this, 'CreateCognitoUser');
    const createProfile = new sfn.Pass(this, 'CreateProfileRecord');
    const createEclipseCustomer = new sfn.Pass(this, 'CreateEclipseCustomer');
    const createEclipseWallet = new sfn.Pass(this, 'CreateEclipseWallet');
    const updateProfile = new sfn.Pass(this, 'UpdateProfileWithEclipseIds');

    const accountWorkflow = new sfn.StateMachine(this, 'AccountProvisioningStateMachine', {
      definitionBody: sfn.DefinitionBody.fromChainable(
        createCognito
          .next(createProfile)
          .next(createEclipseCustomer)
          .next(createEclipseWallet)
          .next(updateProfile)
      ),
      tracingEnabled: true,
      logs: {
    ...
    const eclipseSecret = secretsmanager.Secret.fromSecretCompleteArn(
      ...
      'arn:aws:secretsmanager:af-south-1:732439976770:secret:prod/pashashapay/eclipse-JIpDxA'
    );
