...
DEFAULT_AWS_PROFILE="aws-af-south-1-dev"
...
FRONTEND_STACK_NAME="${FRONTEND_STACK_NAME:-PashashaPayFrontendDevStack}"
...
log() {
  printf "\033[1;34m[deploy-dev]\033[0m %s\n" "$*"
}
...
if [[ -n "${FRONTEND_HOSTED_ZONE_DOMAIN_NAME:-}" ]]; then
  printf "Dev DNS: https://dev.%s\n" "$FRONTEND_HOSTED_ZONE_DOMAIN_NAME"
  printf "WWW Dev DNS: https://www.dev.%s\n" "$FRONTEND_HOSTED_ZONE_DOMAIN_NAME"
fi
