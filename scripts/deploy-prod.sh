...
DEFAULT_AWS_PROFILE="aws-af-south-1-prod"
...
FRONTEND_STACK_NAME="${FRONTEND_STACK_NAME:-PashashaPayFrontendProdStack}"
...
log() {
  printf "\033[1;34m[deploy-prod]\033[0m %s\n" "$*"
}
...
if [[ -n "${FRONTEND_HOSTED_ZONE_DOMAIN_NAME:-}" ]]; then
  printf "Prod DNS: https://%s\n" "$FRONTEND_HOSTED_ZONE_DOMAIN_NAME"
  printf "WWW Prod DNS: https://www.%s\n" "$FRONTEND_HOSTED_ZONE_DOMAIN_NAME"
fi
