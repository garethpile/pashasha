    ...
    <main className="mx-auto flex max-w-5xl flex-col gap-4 px-4 py-6">
      <header className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-600">Search by event type or user.</p>
        </div>
        <button
          type="button"
          onClick={fetchLogs}
      ...
      <section className="grid gap-2 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <div className="divide-y divide-slate-100">
          {logs.map((log) => (
            <div key={log.auditId} className="grid grid-cols-6 gap-3 py-3 text-sm text-slate-800">
              <div className="col-span-2">
                <p className="font-semibold">{log.eventType}</p>
                {log.description && <p className="text-xs text-slate-600">{log.description}</p>}
              </div>
              <div>
                <p className="font-mono text-xs text-slate-700">{log.userId}</p>
              </div>
              <div>
